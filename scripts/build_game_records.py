...
def build_for_team(team: dict[str, Any], target_date: str, games_by_date: dict[str, list[dict[str, Any]]] | None = None) -> None:
    """Build game-record.json for a team's game(s) on target_date."""
    ...
    # Find ALL this team's games on this date (DH support)
    my_games = []
    for g in kbo_games:
    ...
    for game_idx, my_game in enumerate(my_games):
        ...
        # DH 2차전은 _dh2 suffix로 저장
        suffix = "" if game_idx == 0 else f"_dh{game_idx + 1}"
        out_path = ROOT / "data" / "teams" / team["id"] / "game-records" / f"{target_date}{suffix}.json"
