    ...
    const interval = setInterval(async () => {
      ...
      setDetail((prev) => {
        ...
        return {
          ...
          scoreBoard: {
            ...prev.scoreBoard,
            rheb: wg.status === "live" && wg.relay?.inning > 1 ? (wg.scoreBoard?.rheb ?? prev.scoreBoard?.rheb) : prev.scoreBoard?.rheb,
            inn: wg.status === "live" && wg.relay?.inning > 1 ? (wg.scoreBoard?.inn ?? prev.scoreBoard?.inn) : prev.scoreBoard?.inn,
          },
          relay: wg.status === "live" && wg.relay?.inning > 1 ? (wg.relay ?? prev.relay) : prev.relay,
