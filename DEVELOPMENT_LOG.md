...
| 7 | 점수 전부 0:0 | `homeScore`→`homeTeamScore` 필드명 변경 | 23bbf23 |
| 8 | OTA 미수신자 9회말 | inning 배열 9개 전부 0으로 채워짐 | 6ba0aa9 |

#### Naver API 변경 상세 (2026-06-19)

**schedule_games 응답 필드 변경:**

| Naver 필드 (구) | Naver 필드 (신) | 영향 | 대응 |
|-----------------|-----------------|------|------|
| `status` | `statusCode` | 값이 "STARTED", "READY" 등 텍스트로 변경. 숫자 코드("2")도 혼재 | `_naver_status()`에 텍스트+숫자 모두 지원. `g.get("statusCode")`로 읽기 |
| `homeScore` | `homeTeamScore` | 점수 null로 나옴 | `g.get("homeTeamScore")`로 변경 (line 841) |
| `awayScore` | `awayTeamScore` | 점수 null로 나옴 | `g.get("awayTeamScore")`로 변경 (line 842) |
| `homeTeamScoreByInning` | 형식 변경 | `"0,0,1,0,4"` → `['0','2','-','-']` (list string, dash=미진행) | `parse_score_inning`에 `ast.literal_eval` + dash 처리 + 미진행 이닝 truncate |
| `homeTeamRheb` | 형식 변경 | `"5,10,0,2"` → `[0,0,0,0]` (list) | `parse_rheb`에 `ast.literal_eval` 추가 |
| `homeTeamCode` | (빈 값 가능) | 경기전에는 빈 문자열 | gameId에서 regex 파싱 fallback |

**game_relay 응답 구조 변경:**

| 버전 | 구조 | 대응 |
|------|------|------|
| v1 (~6/18) | `{ homeEntry: [...], awayEntry: [...], currentGameState: {...} }` | 리스트 직접 순회, `rd.get("currentGameState")` |
| v2 (6/19 18:31) | `{ textRelayData: { homeEntry: {batter:[], pitcher:[]}, awayEntry: {batter:[], pitcher:[]}, currentGameState: {...}, inn: "1", homeOrAway: "0" } }` | `td.get("currentGameState")`, dict에서 batter+pitcher 합침 |
| v3 (6/19 18:39) | v2 + `homeLineup: {batter:[], pitcher:[]}`, `awayLineup: {batter:[], pitcher:[]}` 추가 | 4개 entry 전부 검색 (homeEntry, awayEntry, homeLineup, awayLineup) |

**서버 수정 파일:**

| 파일 | 수정 사항 |
|------|-----------|
| `main.py` | `statusCode`, `homeTeamScore`/`awayTeamScore`, timezone KST, `_fetch_relay` textRelayData 대응, `_build_game_detail` relay 동일 대응, gameId fallback |
| `scripts/naver_api.py` | `parse_score_inning` list+dash 처리 + truncate, `parse_rheb` list 처리 |
| `scripts/push_worker.py` | 이닝 변경 감지, BSO/주루 제거, weather 30분 push |
