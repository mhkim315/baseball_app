...
### Widget Update Error (2026-06-17)
- **Issue:** The widget UI updates were pushed to the \	est\ branch, but the local device was on the \preview\ build. This caused the widget to not update locally.
- **Fix:** Deployed the OTA update to the \preview\ branch and deleted the errant OTA updates from the \	est\ branch.

### Widget RemoteViews Crash Fix (2026-06-17)
- **Issue:** OTA updates for the widget were downloading successfully but not rendering on the device (4x2 widget was blank, other widgets kept their old layouts). 
- **Cause:** Using layout properties like marginTop and marginHorizontal directly inside the TextWidget component of eact-native-android-widget caused a parsing exception in the native Android Java layer. In Android, when a RemoteViews update encounters an exception, it drops the entire update and retains the last successfully rendered view.
- **Fix:** Removed all margin properties from TextWidget and wrapped the text elements in FlexWidget with the appropriate style properties instead.
- **Result:** Successfully built and deployed OTA updates to both 	est and preview channels. The widgets now render correctly, including the new multi-line rank text and weather info.
