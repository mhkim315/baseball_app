...
def patch_today_games_times():
    """Patch today-games.json time fields with DB games.time values."""
    ...
    try:
        ...
        for game in data.get("nextGames", []):
            game_id = game.get("id", "")
            if not game_id:
                continue
            row = db.execute(
            ...
            if row and row[0]:
                game["time"] = row[0].strip()
                logger.info(f"patched next time {game_id}: {game['time']}")
    except Exception as e:
    ...
    logger.info("today-games.json times patched from DB")
